...
#include <chrono>
#include <vector>
using namespace aurora::terminal;using namespace std::chrono_literals;
...
CHECK(a.input("echo AURORA_ALPHA_ONLY\r"));CHECK(b.input("echo AURORA_BETA_ONLY\r"));
uint64_t ca=0,cb=0;std::string oa,ob;
for(int i=0;i<100;++i){auto x=a.read(ca),y=b.read(cb);CHECK(x&&y);if(x){oa+=x->bytes;ca=x->next;}if(y){ob+=y->bytes;cb=y->next;}if(oa.find("AURORA_ALPHA_ONLY")!=std::string::npos&&ob.find("AURORA_BETA_ONLY")!=std::string::npos)break;std::this_thread::sleep_for(25ms);}
CHECK(oa.find("AURORA_ALPHA_ONLY")!=std::string::npos);CHECK(ob.find("AURORA_BETA_ONLY")!=std::string::npos);CHECK(oa.find("AURORA_BETA_ONLY")==std::string::npos);CHECK(ob.find("AURORA_ALPHA_ONLY")==std::string::npos);
CHECK(a.input("cd /d %SystemRoot%\r"));CHECK(a.input("echo AURORA_DIR=%CD%\r"));CHECK(a.input("start /b ping.exe -t 127.0.0.1 >nul\r"));
...
for(int i=0;i<40;++i){auto x=a.read(ca);if(x){oa+=x->bytes;ca=x->next;}if(oa.find("AURORA_DIR=C:\\Windows")!=std::string::npos)break;std::this_thread::sleep_for(25ms);}
CHECK(oa.find("AURORA_DIR=")!=std::string::npos);
...
Session c({.initial_command="echo AURORA_INIT"});
...
for(int i=0;i<100;++i){auto x=c.read(cc);if(x){oc+=x->bytes;cc=x->next;}if(oc.find("AURORA_INIT")!=std::string::npos)break;std::this_thread::sleep_for(25ms);}
CHECK(oc.find("AURORA_INIT")!=std::string::npos);
