...
## Starting-page capabilities

The exact internal main-frame URLs opengod://newtab and opengod://newtab/ may send version 1 commands `cveState`, `refreshCves`, `createTerminal`, `launchOmniroute`, `macroState`, `saveMacro`, `deleteMacro`, `launchMacro`, `sessionPersistenceState`, and `setSessionPersistence`. Requests are bounded at 4,096 characters. The native Client checks its live tab/browser identity, main frame, exact URL, nonpersistent request and shutdown status before parsing. Web pages and internal subframes have no bridge. A starting-page request never enters the shell dispatcher.

**Website-session preference** (`sessionPersistenceState`, `setSessionPersistence`): The state command takes only `version` and `command`. The mutation additionally requires a boolean `enabled` and rejects missing, extra or wrongly typed fields without changing the setting. Both return `{version: 1, enabled: boolean, active: boolean}`. `enabled` is the saved preference; `active` is the value applied to CEF when this browser process started. The preference defaults to false and is stored as a per-user Windows registry DWORD under `HKCU\Software\OpenGod\SaveLoginSessions`. Off uses an in-memory CEF default profile; on uses the persistent `Default` cache path with session-cookie retention. A difference between the two requires a full OpenGod restart. A failed registry write returns an error and leaves the displayed preference unchanged. Turning off does not delete previously stored website data, and tabs are not restored.

**CVE feed** (`cveState`, `refreshCves`): The response contains version, status (loading/ready/stale/error), message, UTC start/end/fetched timestamps and entries. Each entry carries id, description, published, modified, nullable numeric score, cvssVersion, severity and assessmentSource. C++ owns filtering, ordering and cache policy; TypeScript validates and presents the snapshot in groups of 50. Navigating or closing a page cancels its frontend polling; shared native refreshes can finish for other tabs. CVE ordering uses highest score first (descending), then CVE ID for ties, with unscored entries last. The CVE feed covers today: midnight UTC through the current fetch start time. Cache reuse is limited to the same UTC date; previous-day entries are cleared when a new-day refresh begins. See CVE-FEED.md for policy and limits.

**Terminal and macros** (`createTerminal`, `launchOmniroute`, `macroState`, `saveMacro`, `deleteMacro`, `launchMacro`): `createTerminal` and `launchOmniroute` take no additional fields. `macroState` returns `{version: 1, macros: [{id, name, terminalCommand, url}, ...]}`. `saveMacro` requires `name` (1–40 chars), `terminalCommand` (0–256 chars, no control chars or shell metacharacters `&|<>;%"`), and `url` (optional, validated by `resolve_navigation`); optionally includes `macroId` to update an existing macro. `deleteMacro` requires positive integer `macroId`. `launchMacro` requires positive integer `macroId` and launches the named macro (terminal command, then URL, both optional). All macro responses return the updated macro list. Macros are in-memory, session-lifetime; persistence is a future profile milestone. Unknown macro IDs, invalid field types, forbidden command characters, oversized fields and macros with neither command nor URL all fail without changing state.

## New-tab focus

New browsing-view creation sends the trusted shell `opengod-new-tab-address`, with the native tab ID as its numeric detail. The shell refreshes validated state and focuses/selects the address only if that tab is still active and is not a terminal. A successful shell `createTab` response also focuses the address for a new-tab page, and the initial content load repeats the native handoff in case the view took focus during creation. Native CEF focus transfers to the shell; navigation-origin focus requests from the starting page are suppressed, while user focus requests remain allowed. Terminal creation does not send this event. Existing tab activation does not refocus the address.
