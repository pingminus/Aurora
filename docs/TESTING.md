...
## Bookmark rename checks

Core coverage includes unchanged ID/URL, unknown and removed IDs, empty and ASCII-whitespace-only titles, the 256-byte UTF-8 boundary, and renaming after closing the source tab. UI bridge coverage verifies title transport, rejection recovery and literal markup/Unicode in returned titles. Native validation requires the exact rename envelope; malformed native envelopes, light/high-contrast layouts and rapid remove/rename races still need dedicated runtime checks.

Verification for this change: npm ci, typecheck, all 18 UI tests and both standalone core CTest suites passed. Native Release compilation and both packaged CTest suites passed in build/rename-check. The existing build/browser DLL was locked by the running browser, so that package was not replaced. An isolated-profile test package launched and rendered its shell and starting page at 1360 by 900. Automated input was repeatedly interrupted by user input; native Save/Cancel/repeated-edit behavior and real website rendering were not verified in this run.

Follow-up build update: the normal build/browser Release package rebuilt successfully after the browser exited. Both packaged core CTest suites passed. The executable, DLL and UI assets were refreshed in build/bin/Release as well; both launch locations have identical native DLL hashes. This rebuild does not close the interactive verification gaps above.
