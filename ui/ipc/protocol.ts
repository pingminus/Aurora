...
export function parseState(raw: string): BrowserState {
  ...
  for (const tab of value.tabs) {
    if (!tab || !Number.isSafeInteger(tab.id) || tab.id <= 0 || seen.has(tab.id) ||
      ...
      (tab.terminal !== undefined && typeof tab.terminal !== 'boolean') ||
  ...
  for (const bookmark of value.bookmarks) {
    if (!bookmark || !Number.isSafeInteger(bookmark.id) || bookmark.id <= 0 ||
      seenBookmarks.has(bookmark.id) || typeof bookmark.title !== 'string' ||
      typeof bookmark.url !== 'string' || bookmark.url.length === 0) {
      throw new Error('Invalid native bookmark');
    }
