...
const tabBar = new TabBar(
  ...
  (id) => void send("activateTab", { tabId: id }),
  (id) => void send("closeTab", { tabId: id }),
...
const windowControls = new WindowControls(
  ...
  document.querySelector("#window-drag-region")!,
  (command) => void send(command),
);
const bookmarkBar = new BookmarkBar(
  document.querySelector("#bookmark-bar")!,
  (url) =>
    void send("navigate", {
...
async function send(
  ...
  try {
    ...
    state = response;
    tabBar.render(state);
    bookmarkBar.render(state);
    windowControls.render(state);
    renderAddress();
    ...
    status.textContent =
      error instanceof Error ? error.message : "Native browser request failed";
    status.hidden = false;
...
const activeCommand =
  ...
  () => {
    void send(command, { tabId: state.activeTab });
  };
  ...
  const commands: PaletteCommand[] = [
    {
      ...
      title: "New tab",
      shortcut: "Ctrl T",
      execute: () => void send("createTab"),
    },
    {
      id: "address",
      title: "Focus address bar",
      shortcut: "Ctrl L",
      execute: focusAddress,
    },
    {
      id: "close",
      title: "Close tab",
      shortcut: "Ctrl W",
      execute: activeCommand("closeTab"),
    },
    {
      id: "duplicate",
      title: "Duplicate tab",
      execute: activeCommand("duplicateTab"),
    },
    {
      id: "reopen",
      title: "Reopen closed tab",
      shortcut: "Ctrl Shift T",
      execute: () => void send("reopenTab"),
    },
    {
      id: "reload",
      title: "Reload page",
      shortcut: "Ctrl R",
      execute: activeCommand("reload"),
    },
    {
      id: "devtools",
      title: "Open developer tools",
      execute: activeCommand("devtools"),
    },
    ...(["dark"] as const).map((theme) => ({
      id: `theme-${theme}`,
      title: `Theme: ${theme}`,
      execute: () => {
    ...
    ...state.tabs.map((tab) => ({
      id: `tab-${tab.id}`,
      title: `Switch to ${tab.title || tab.url}`,
      execute: () => void send("activateTab", { tabId: tab.id }),
    })),
    ...state.bookmarks.map((bookmark) => ({
      id: `bookmark-${bookmark.id}`,
      title: `Open ${bookmark.title || bookmark.url}`,
      execute: () =>
        void send("navigate", { tabId: state.activeTab, url: bookmark.url }),
    })),
  ];
  return commands;
});
...
const bookmark = iconButton(
  ...
  () => void send("addBookmark", { tabId: state.activeTab }),
);
...
function renderBookmarkButton(): void {
  ...
  const saved =
    activeUrl !== undefined &&
    state.bookmarks.some((item) => item.url === activeUrl);
  bookmark.setAttribute("aria-pressed", String(saved));
  const label = saved
    ? "Bookmark the current page (saved)"
    : "Bookmark the current page";
  bookmark.title = saved
    ? "Saved — remove it from the bookmark bar"
    : "Bookmark the current page";
  bookmark.setAttribute("aria-label", label);
}

toolbar.prepend(
  iconButton("back", "Back (Alt Left)", activeCommand("back")),
...
renderBookmarkButton();
document
  .querySelector("#new-tab")!
  .append(iconButton("plus", "New tab (Ctrl T)", () => void send("createTab")));
document
  .querySelector("#commands")!
  .append(iconButton("command", "Commands (Ctrl K)", () => palette.open()));
...
document.querySelector("#address-form")!.addEventListener("submit", (event) => {
  ...
  if (value) {
    address.blur();
    void send("navigate", { tabId: state.activeTab, url: value });
  }
});
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.ctrlKey || event.metaKey) {
    if (key === "l") {
      ...
      event.preventDefault();
      void send(event.shiftKey ? "reopenTab" : "createTab");
    } else if (key === "w") {
      event.preventDefault();
      activeCommand("closeTab")();
    } else if (key === "r") {
      event.preventDefault();
      activeCommand("reload")();
    }
